...
Most ML pipelines are batch jobs: collect data, run inference on a schedule, write results somewhere downstream. That's fine until a prediction is only useful within seconds of the event that triggered it, like fraud scoring, live personalization, or anomaly detection. I built a [real-time ML inference pipeline](https://github.com/pkrishna1801/ML-Streaming-Pipeline) on AWS, provisioned entirely through Terraform, for exactly that case.
